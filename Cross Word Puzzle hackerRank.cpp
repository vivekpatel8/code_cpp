...
Suppose we have  words, then start from first word and try to place it at valid position in the grid, either horizontally or vertically, and then move ahead with the updated grid for the next word. If for any word there is no valid position found, then backtrack and restore the previous grid(i.e, grid before when last word is not placed) and so on.
